...
public class CopyCab extends BaseFileCab {
    ...
    public Spanned getTitle() {
        if (getFiles().size() == 1)
            return Html.fromHtml(getMainActivity().getString(R.string.copy_x, getFiles().get(0).getName()));
        return Html.fromHtml(getMainActivity().getString(R.string.copy_x, getFiles().size() + ""));
    }
    ...
    public boolean onCabCreated(MaterialCab materialCab, Menu menu) {
        ...
        if (getFragment() != null && getDirectory().getParent() != null) {
        ...
        BackgroundThread.getHandler().post(new Runnable() {
            ...
            public void run() {
                for (int i = 0; i < getFiles().size(); i++) {
                    ...
                    final File result;
                    try {
                        result = file.copy(getMainActivity(), newFile, mDialog);
